...
3. **HTML Report Generation** _(production branches only)_ — fetches up to 10,000 issues and up to 10,000 hotspots (SonarQube's per-query API limit), then renders a self-contained HTML report. A warning banner is shown when either result set was capped.
