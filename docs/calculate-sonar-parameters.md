...
The fetch serves both scan paths: the SonarQube Runner meta-runner, and the Gradle or Maven plugin, which scans Java/Kotlin components in another step while `SKIP_SONAR_METARUNNER_EXECUTION` is `true`. The step therefore always runs.
